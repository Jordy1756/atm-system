...
export default function ModeloColasMMM() {
    ...
    const clientesPromedioEnCola =
        (probabilidadSistemaVacio *
            Math.pow(tasaLlegadaClientes / tasaAtencionPorCajero, cantidadCajeros) *
            tasaUtilizacionSistema) /
        (factorial(cantidadCajeros) * Math.pow(1 - tasaUtilizacionSistema, 2));
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            <h1 className="text-2xl font-bold mb-6 text-center">
                Modelo M/M/m - Evaluación Económica del Sistema de Cajeros
            </h1>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <Input
                    label="Tasa de llegada de clientes por hora (λ)"
                    value={tasaLlegadaClientes}
                    setValue={setTasaLlegadaClientes}
                />
                <Input
                    label="Tasa de atención por cajero por hora (μ)"
                    value={tasaAtencionPorCajero}
                    setValue={setTasaAtencionPorCajero}
                />
                ...
                <Input
                    label="Costo por cajero (₡/hora)"
                    value={costoPorCajeroPorHora}
                    setValue={setCostoPorCajeroPorHora}
                />
                <Input
                    label="Costo de espera por cliente (₡/cliente/hora)"
                    value={costoEsperaPorClientePorHora}
                    setValue={setCostoEsperaPorClientePorHora}
                />
                <Input
                    label="Ticket promedio por cliente (₡)"
                    value={ticketPromedioPorCliente}
                    setValue={setTicketPromedioPorCliente}
                />
                <Input
                    label="Margen de ganancia (%)"
                    value={margenGananciaPorVenta}
                    setValue={setMargenGananciaPorVenta}
                    step={0.01}
                />
            </div>
                ...
                <table className="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <TableRow
                            ...
                            formula="Lq / λ"
                            value={`${tiempoPromedioEnCola.toFixed(4)} horas (${(tiempoPromedioEnCola * 60).toFixed(
                                2
                            )} min)`}
                        />
                        <TableRow
                            ...
                            formula="L / λ"
                            value={`${tiempoPromedioTotalEnSistema.toFixed(4)} horas (${(
                                tiempoPromedioTotalEnSistema * 60
                            ).toFixed(2)} min)`}
                        />
                        <TableRow
        ...
        <div className="flex flex-col h-full">
            <div className="flex-grow flex items-end space-x-4 px-4">
                <div className="flex-1 flex flex-col items-center">
                    <div className="w-1/2 bg-green-400" style={{ height: `${(ingresos / total) * 100}%` }}></div>
                    <span className="text-xs mt-1">Ingresos</span>
                    <span className="text-xs font-semibold">₡{ingresos.toFixed(2)}</span>
                </div>
                <div className="flex-1 flex flex-col items-center">
                    <div className="w-1/2 bg-red-400" style={{ height: `${(costos / total) * 100}%` }}></div>
